...
# ℹ️ Для NVIDIA: КРИТИЧЕСКИ ВАЖНО для работы Wayland и энергосбережения!
# Без этих настроек гибернация и Wayland могут не работать.
# ⚠️ Если вы выбрали СЦЕНАРИЙ А или Б — ПРОПУСТИТЕ этот шаг!
#------------------------------------------------------------------------------
# 📋 1. Добавить модули NVIDIA в mkinitcpio.conf (ТОЛЬКО ДЛЯ NVIDIA)
# ✅ Учитывает закомментированную строку по умолчанию (#MODULES=()) в Arch.
# ✅ Корректно захватывает содержимое скобок и добавляет драйверы.
...
# ✅ Учитывает закомментированную строку по умолчанию (#GRUB_CMDLINE...) в Arch.
# ✅ Сохраняет существующие параметры и корректно обрабатывает кавычки.
...
# ✅ Принудительная загрузка модуля nvidia-drm при старте.
echo 'nvidia-drm' > /etc/modules-load.d/nvidia-drm.conf
# 📋 4. Пересобрать initramfs
# ✅ Применяет изменения модулей. Для DKMS компилирует драйвер под текущее ядро.
mkinitcpio -P
# 📋 5. Обновить конфигурацию GRUB
# ✅ Генерирует новый загрузочный конфиг с добавленными параметрами.
grub-mkconfig -o /boot/grub/grub.cfg
# 🧹 Очистка экрана терминала
clear
echo " "
...
#------------------------------------------------------------------------------
echo " 🔍 Проверка MODULES в mkinitcpio.conf"
grep "^MODULES=" /etc/mkinitcpio.conf | grep -q "nvidia" && echo "✅ Модули NVIDIA добавлены в mkinitcpio" || echo "⚠️ Модули NVIDIA не найдены"
echo " 🔍 Проверка параметров ядра в grub.cfg"
