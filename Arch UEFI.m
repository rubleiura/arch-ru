...
# 🔐 Инициализация ключей pacman для проверки пакетов
# pacman-key --init — создание ключей для проверки пакетов
# pacman-key --populate — загрузка ключей Arch Linux
pacman-key --init
...
# --bootloader-id="Grub Arch Linux" — имя в меню UEFI
grub-install --efi-directory=/boot --boot-directory=/boot --bootloader-id="Grub Arch Linux"
# 📋 Настройка GRUB — включение splash экрана
sed -i '/GRUB_CMDLINE_LINUX_DEFAULT/s/quiet/quiet splash/' /etc/default/grub
# 📋 Настройка grub-btrfs для отображения снапшотов
sed -i "s/#GRUB_BTRFS_SUBMENUNAME=/GRUB_BTRFS_SUBMENUNAME=/" /etc/default/grub-btrfs/config
sed -i "s/#GRUB_BTRFS_TITLE_FORMAT=/GRUB_BTRFS_TITLE_FORMAT=/" /etc/default/grub-btrfs/config
...
# 📌 Раскомментируйте строки для ваших видео-карт
...
#####################################################
## 🧪 ШАГ 4: ДЛЯ ПРОВЕРКИ РАБОТЫ ВИДЕО-КАРТ        ##
#####################################################
# ℹ️ Зачем: Убедиться, что драйверы работают корректно.
# ❗ Важно: Проверка выполняется ТОЛЬКО после загрузки в новую систему.
