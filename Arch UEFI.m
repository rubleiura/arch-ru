...
# │    ├─ ESP — EFI System Partition (раздел для загрузчика)        │
...
# 5.  Разметка диска GPT + UEFI (Блок 4)
...
def get_ip_location():
    ...
    # ✅ Используем ipapi.co с бесплатным HTTPS
    apis = [
...
#################################################################
# ℹ️ Зачем: Создание разделов: ESP (EFI), root, swap.
...
# mkfs.fat -F32 — создание файловой системы FAT32
...
# /boot — куда будет установлен загрузчик UEFI
...
# ⚠️ В UEFI версии используется memtest86+-efi (с -efi)
...
################################################################
# 🔧 [CHROOT] БЛОК 10: УСТАНОВКА ЯДРА, GRUB, MKINITCPIO (UEFI)
#################################################################
# ℹ️ Зачем: Настройка загрузчика и initramfs для UEFI.
# 💡 Включает: GRUB, grub-btrfs, efibootmgr, resume из swap.
...
pacman -Sy --needed --noconfirm grub grub-btrfs efibootmgr os-prober
...
# ИСПРАВЛЕНИЯ:
# 1. Используется .* для замены всей строки
# 2. Внутренние кавычки экранированы как \"
# 3. Гарантирует корректную передачу rootflags и resume
...
echo " "
echo "#####################################################"
echo "## ✅ ЗАГРУЗЧИК И ЯДРО УСТАНОВЛЕНЫ (UEFI)          ##"
echo "#####################################################"
echo " "
