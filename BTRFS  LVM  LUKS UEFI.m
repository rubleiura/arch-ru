...
#----------------------------------------------------------------
# ℹ️ Назначение Блока 14: Установка драйверов Intel/AMD/NVIDIA, настройка
#    ядерных модулей для Wayland, гибернации и энергосбережения.
# 💡 Важно: Для NVIDIA критически важна настройка KMS и служб питания.
...
#------------------------------------------------------------------------------
# ℹ️ Эти пакеты уже зависят от базовой mesa (установленной в Блоке 13),
#    поэтому устанавливаем только инструменты диагностики, мониторинга и бенчмаркинга.
...
# ℹ️ ВАЖНО: Поддержка VA-API (аппаратное ускорение видео) для AMD уже включена 
...
# 📦 nvidia-dkms        : Проприетарный драйвер (максимальная стабильность).
...
#
# pacman -S --noconfirm nvidia-dkms nvidia-utils lib32-nvidia-utils egl-wayland egl-wayland2 nvidia-settings
#
...
# 📦 2. Установка инструмента управления питанием dGPU (выберите ОДИН метод)
# Метод 1: switcheroo-control (современный, интеграция в GUI, рекомендуется для Wayland)
# pacman -S --noconfirm switcheroo-control
# Метод 2: nvidia-prime (запуск через prime-run в терминале, раскомментируйте при необходимости)
...
#   (systemctl enable nvidia-suspend.service и т.п.) – приведёт к kernel panic на гибридах!
#
# • Создавать /etc/modprobe.d/nvidia.conf с опцией NVreg_PreserveVideoMemoryAllocations=1
#   (эта опция предназначена ТОЛЬКО для ПК с одной NVIDIA! На гибридных ноутбуках она вызовет
#   конфликты с Runtime PM и приведёт к чёрному экрану при пробуждении).
#
# • Использовать устаревшие скрипты с rmmod nvidia перед сном (современные драйверы 550+
#   корректно обрабатывают это через параметры ядра без принудительной выгрузки модулей).
#
# ✅ ПРАВИЛЬНАЯ НАСТРОЙКА ДЛЯ ГИБРИДНЫХ НОУТБУКОВ:
# • Использовать параметр ядра nvidia.NVreg_PreserveVideoMemoryAllocations=0 в GRUB
#   (это уже сделано в команде sed выше в Шаге 3)
# • Параметр =0 запрещает драйверу сохранять состояние VRAM при уходе в сон/гибернацию,
#   так как на гибридных ноутбуках дискретная карта полностью обесточивается.
# • Попытка сохранить состояние VRAM (параметр =1) на гибриде приводит к kernel panic
#   или чёрному экрану при пробуждении, так как dGPU уже недоступна.
# ==============================================================================
...
#   проприетарные драйверы (nvidia-dkms) и свежие ядра для стабильной
