...
# ===============================================================================
# Способ визуальной настройки кликами мыши. Адаптировано под эталонный конфиг.
# ===============================================================================
...
# - Найдите параметр "TLP_AUTO_SWITCH" и выберите значение "2" (Smart-переключение).
# - Найдите параметр "TLP_PROFILE_AC" и выберите "PRF" (Performance — максимальная мощность).
# - Найдите параметр "TLP_PROFILE_BAT" и выберите "BAL" (Balanced — баланс от батареи).
# - Найдите параметр "TLP_PROFILE_DEFAULT" и выберите "BAL".
...
#      - В пунктах "_ON_BAT" и "_ON_SAV" выберите "powersave".
...
# ШАГ 5: РАЗДЕЛЕНИЕ ПО КОНФИГУРАЦИЯМ ВИДЕОКАРТ
...
#
# Настройте графику в зависимости от вашей видеокарты:
# ---> СЦЕНАРИЙ 1: ОДИНОЧНАЯ КАРТА INTEL: "Runtime PM" -> "RUNTIME_PM_ON_BAT" -> выберите "auto".
# ---> СЦЕНАРИЙ 2: ОДИНОЧНАЯ КАРТА AMD: "Graphics" -> AMDGPU_ABM_LEVEL (0/1/3), "Runtime PM" -> "auto".
# ---> СЦЕНАРИЙ 3: ОДИНОЧНАЯ КАРТА NVIDIA: "Runtime PM" -> "RUNTIME_PM_ON_BAT" -> выберите "auto".
# ---> СЦЕНАРИЙ 4 & 5: ГИБРИДНАЯ ГРАФИКА (INTEL+NVIDIA / AMD+NVIDIA):
#      - Вкладка "Runtime PM" -> найдите параметр "RUNTIME_PM_DRIVER_DENYLIST".
#        ОБЯЗАТЕЛЬНО удалите из этой строки слова "nvidia" и "nouveau" (а для AMD процессоров и "amdgpu").
...
# === [БЛОК 3: РАЗДЕЛЕНИЕ ПО КОНФИГУРАЦИЯМ ВИДЕОКАРТ] ===
# ---> ДЛЯ ГИБРИДНЫХ СИСТЕМ INTEL + NVIDIA (Две видеокарты):
# RUNTIME_PM_DRIVER_DENYLIST="amdgpu mei_me xhci_hcd"
...
# Чтобы вернуть оригинальный эталонный файл, в котором абсолютно все строки закомментированы решёткой #:
#
# 1. Сначала полностью гасим и останавливаем запущенные в фоне службы управления питанием и радиосетями:
sudo systemctl disable --now tlp.service tlp-pd.service NetworkManager-dispatcher.service

# 2. Физически удаляем старый измененный конфигурационный файл с диска:
sudo rm -f /etc/tlp.conf

# 3. Заставляем pacman начисто перезаписать файл конфигурации оригиналом из репозитория:
sudo pacman -S --noconfirm tlp tlp-rdw

# 4. Активируем и запускаем службы заново (они подхватят уже дефолтный файл):
sudo systemctl enable tlp.service
...
# (Если настраивается ПК с одной видеокартой, закомментируйте две строки ниже)
...
# (Профили работы, пороги заряда батареи, радиоустройства и шина PCIe)
...
# ВНИМАНИЕ: Оставьте в вашем итоговом .sh скрипте только ОДИН из двух блоков ниже.

# ---> ПОДБЛОК ДЛЯ СИСТЕМ НА БАЗЕ AMD (Ryzen):
# (Раскомментируйте строки ниже, если у вас процессор AMD)
...
# (Раскомментируйте строки ниже, если у вас процессор Intel)
# sudo sed -i 's/^#CPU_DRIVER_OPMODE_ON_AC=.*/CPU_DRIVER_OPMODE_ON_AC=active/' /etc/tlp.conf
# sudo sed -i 's/^#CPU_DRIVER_OPMODE_ON_BAT=.*/CPU_DRIVER_OPMODE_ON_BAT=active/' /etc/tlp.conf
# sudo sed -i 's/^#CPU_DRIVER_OPMODE_ON_SAV=.*/CPU_DRIVER_OPMODE_ON_SAV=active/' /etc/tlp.conf
...
# === [БЛОК 3: АВТОМАТИЗАЦИЯ НАСТРОЕК ВИДЕОКАРТ] ===
# Раскомментируйте и примените строки, строго соответствующие вашей видеосистеме:

# ---> ДЛЯ КОМПЬЮТЕРОВ С УЧАСТИЕМ ГРАФИКИ INTEL (Одиночная карта ИЛИ гибрид Intel+NVIDIA):
# sudo sed -i 's/^#INTEL_GPU_MIN_FREQ_ON_BAT=.*/INTEL_GPU_MIN_FREQ_ON_BAT=300/' /etc/tlp.conf
# sudo sed -i 's/^#INTEL_GPU_MAX_FREQ_ON_BAT=.*/INTEL_GPU_MAX_FREQ_ON_BAT=600/' /etc/tlp.conf

# ---> ДЛЯ КОМПЬЮТЕРОВ С УЧАСТИЕМ ГРАФИКИ AMD (Одиночная карта ИЛИ гибрид AMD+NVIDIA):
# sudo sed -i 's/^#AMDGPU_ABM_LEVEL_ON_AC=.*/AMDGPU_ABM_LEVEL_ON_AC=0/' /etc/tlp.conf
# sudo sed -i 's/^#AMDGPU_ABM_LEVEL_ON_BAT=.*/AMDGPU_ABM_LEVEL_ON_BAT=1/' /etc/tlp.conf
# sudo sed -i 's/^#AMDGPU_ABM_LEVEL_ON_SAV=.*/AMDGPU_ABM_LEVEL_ON_SAV=3/' /etc/tlp.conf

# ---> ДЛЯ ГИБРИДНЫХ СИСТЕМ С ДВУМЯ ВИДЕОКАРТАМИ (Intel+NVIDIA):
# (Активирует динамический сон для дискретной NVIDIA, исключая её из блокировок TLP)
# sudo sed -i 's/^#RUNTIME_PM_DRIVER_DENYLIST=.*/RUNTIME_PM_DRIVER_DENYLIST="amdgpu mei_me xhci_hcd"/' /etc/tlp.conf

# ---> ДЛЯ ГИБРИДНЫХ СИСТЕМ С ДВУМЯ ВИДЕОКАРТАМИ (AMD+NVIDIA):
# (Исключает из блокировок сна как драйвер встроенной AMD, так и дискретной NVIDIA)
# sudo sed -i 's/^#RUNTIME_PM_DRIVER_DENYLIST=.*/RUNTIME_PM_DRIVER_DENYLIST="mei_me xhci_hcd"/' /etc/tlp.conf
...
# (Удалите строку ниже, если в целевом ПК всего одна видеокарта)
switcherooctl list
...
# -------------------------------------------------------------------------------
# Полный и безопасный алгоритм программного отката на оригинальный закомментированный файл:
#
# 1. Сначала полностью гасим и останавливаем запущенные в фоне службы управления питанием и радиосетями:
# sudo systemctl disable --now tlp.service tlp-pd.service NetworkManager-dispatcher.service
#
# 2. Физически удаляем измененный конфигурационный файл:
# sudo rm -f /etc/tlp.conf
#
# 3. Заставляем менеджер пакетов pacman заново скачать и накатить чистый эталонный файл:
# sudo pacman -S --noconfirm tlp tlp-rdw
#
# 4. Включаем и запускаем системные службы обратно:
# sudo systemctl enable tlp.service
# sudo systemctl enable --now tlp-pd.service NetworkManager-dispatcher.service
# sudo tlp start
