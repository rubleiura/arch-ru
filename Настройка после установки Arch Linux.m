...
################################################################################
# ЧЕК-ЛИСТ НАСТРОЙКИ WINETRICKS В LUX WINE (ДЛЯ ИГР WINDOWS 10 / 11)
################################################################################

# ШАГ 1: ПОДГОТОВКА И БАЗОВАЯ НАСТРОЙКА WINECFG
#   - Открыть Lux Wine -> Выбрать игровой префикс -> Winetricks.
...
vcrun2012
vcrun2013
vcrun2015 (или vcrun2022, если доступен — включает в себя 2015-2022).
...
#   - В Winetricks выберите "Сlean up" (если доступно) для удаления кэша инсталляторов.

################################################################################
# ВАЖНОЕ ПРАВИЛО: Не отмечайте всё подряд галочками одновременно!
# Устанавливайте компоненты группами (сначала шрифты, затем vcrun, затем DirectX),
...
################################################################################
# Термин GameMode (игровой режим) может относиться к разным вещам в зависимости от контекста

# 1. GameMode, это встроенная функция, которая повышает производительность за счет выделения
# максимальных ресурсов процессору и видеокарте. Она также временно отключает фоновые процессы
# и уведомления. Вы можете управлять им через системные параметры ПК.
# 2. Утилита GameMode в LinuxВ дистрибутивах Linux (включая SteamOS на консолях Steam Deck)
# GameMode — это фоновая утилита . Она автоматически переключает процессор в режим высокой
# производительности на время игры и возвращает его в энергосберегающий режим после закрытия.
# Шаг 1 Установка
sudo pacman -S --needed --noconfirm gamemode
# Шаг 2. Добавление пользователя в группу gamemode
sudo usermod -aG gamemode $USER
# Шаг 3. Обязательно перезагрузите компьютер
reboot
# Шаг 4. Финальная проверка
gamemoded -t

# Как правильно запустить игру в Lux-Wine с GameMode

# Способ А: Через ярлык игры (Самый удобный)
# 1. Нажмите правой кнопкой мыши по созданному ярлыку игры и выберите Свойства
# 2. Найдите строку Команда (или Исполняемый файл / Target).
# 3. Там будет длинная строка, начинающаяся с пути к Lux-Wine. Вам нужно просто вписать команду gamemoderun в самое начало этой строки (через пробел перед всем остальным текстом).

# Пример было:
"/home/user/Lux-Wine/lux-wine" run "/path/to/game.exe"
/home/user/.local/share/LuxWine/bin/lwrun '/path/to/game.exe'
# Пример стало:
gamemoderun /home/user/.local/share/LuxWine/bin/lwrun '/path/to/game.exe'

# Способ Б: Через Терминал (Для проверки)

# Пример
gamemoderun lwrun "/путь/к_папке_с_игрой/game.exe"
