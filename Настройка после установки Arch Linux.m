...
# Для того чтобы большинство Windows-игр запускались через Lux Wine без проблем, необходимо установить базовый набор библиотек, шрифтов и компонентов DirectX через встроенный Winetricks.

# Пошаговый алгоритм настройки.

## 1. Подготовка чистого префикса

# Перед установкой компонентов убедиться, что работаете с нужным префиксом (желательно 64-битным для современных игр).

# * Открыть Lux Wine.
# * Выбрать игровой префикс (или создать новый).
# * Открыть Winetricks через меню управления этим префиксом.

## 2. Установка шрифтов (Fonts)

# Многие игры и их лаунчеры вылетают, если в системе отсутствуют стандартные шрифты Windows.

# * В Winetricks выбрать Select the default wineprefix -> Install a font.
# * Отметить галочкой и установить:
...
# * Отметить галочками и установить следующий универсальный набор:
...
# * В том же списке DLL найти и установить:
...
# * Выйти в главное меню Winetricks для этого префикса.
# * Выбрать Run winecfg.
# * На вкладке «Приложения» (Applications) снизу установить Версию Windows: Windows 10 (для старых игр начала 2000-х можно временно менять на Windows XP/7).
