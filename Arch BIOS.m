...
# 📦 grub-btrfs — интеграция снапшотов BTRFS в меню GRUB (~0.2 МБ)
...
# NetworkManager — автоматическое управление сетью
# grub-btrfsd — демон для обновления меню GRUB при создании снапшотов
# sshd — SSH сервер для удалённого доступа
...
# resume — позволяет восстановить систему из гибернации (swap)
# Хук должен идти ПОСЛЕ block/udev и ДО filesystems
# Команда добавляет resume в правильное место автоматически
sed -i '/^HOOKS=/{s/\bblock\b/block resume/; t; s/\budev\b/udev resume/}' /etc/mkinitcpio.conf
# 📋 Добавление модулей ядра в initramfs
# btrfs — поддержка файловой системы BTRFS на раннем этапе загрузки
sed -i "s/MODULES=()/MODULES=(btrfs)/" /etc/mkinitcpio.conf
# 📋 Получение UUID swap раздела для resume
# UUID используется вместо имени устройства для надёжности
# ⚠️ Убедитесь, что sda3 соответствует вашему swap разделу
SWAP_UUID=$(blkid -s UUID -o value /dev/sda3)
# 📋 Настройка GRUB_CMDLINE_LINUX_DEFAULT
# loglevel=3 — уменьшает количество сообщений при загрузке
# quiet — скрывает вывод ядра
# splash — включает графический экран загрузки (plymouth)
# rd.shell=0 — отключает аварийную оболочку dracut
# rd.emergency=halt — останавливает систему при критической ошибке
...
# rootflags=subvol=@ — монтирует подтом @ как корень
# resume=UUID=... — указывает раздел для восстановления из гибернации
# ИСПРАВЛЕНИЯ:
# 1. Используется .* для замены всей строки (вместо ошибочной точки)
# 2. Внутренние кавычки экранированы как \" (предотвращает синтаксические ошибки)
# 3. Гарантирует корректную передачу параметров ядру
...
# part_gpt/part_msdos — поддержка таблиц разделов GPT и MBR
...
# Включение подменю со снапшотами в меню загрузки
...
# Создаёт файл /boot/grub/grub.cfg на основе настроек
grub-mkconfig -o /boot/grub/grub.cfg
# 📋 Пересборка initramfs
# Применяет изменения в mkinitcpio.conf и создаёт образы initramfs
mkinitcpio -P
clear
echo " "
echo "#####################################################"
...
# pacman -Sy --needed --noconfirm vulkan-intel  # Раскомментируйте для CPU ≤ 11 gen
...
# 2. ✅ Добавить параметр ядра nvidia-drm.modeset=1
# Включает KMS (Kernel Mode Setting) для NVIDIA.
# Требуется для Wayland и корректной гибернации.
...
# Применяет изменения параметров ядра
grub-mkconfig -o /boot/grub/grub.cfg
