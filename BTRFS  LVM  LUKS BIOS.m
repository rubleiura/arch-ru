...
#################################################################
# ℹ️ Зачем: Подготовка разделов, настройка шифрования LUKS,
# создание логических томов LVM, разметка Btrfs и монтирование.
# 💡 Структура:
#    /dev/sda2 -> ext4 (/boot, не шифруется)
#    /dev/sda3 -> LUKS -> LVM -> Btrfs (/, /home, swap)
# ❗ ВАЖНО: Запомните пароль LUKS! Без него данные не восстановить.
# ⚠️ ПЕРЕД ВЫПОЛНЕНИЕМ: Убедитесь, что sda1/sda2/sda3 заменены
...
# mkfs.ext4 -L BOOT — создание ФС ext4 с меткой BOOT.
#------------------------------------------------------------------------------
# 📦 Форматирование /boot раздела в ext4
mkfs.ext4 -L BOOT /dev/sda2
#------------------------------------------------------------------------------
# ШАГ 2: НАСТРОЙКА ШИФРОВАНИЯ LUKS
#------------------------------------------------------------------------------
# LUKS (Linux Unified Key Setup) обеспечивает полное шифрование раздела.
# cryptsetup luksFormat — инициализация контейнера (ДАННЫЕ БУДУТ УТЕРЯНЫ!).
# cryptsetup open — открытие контейнера и создание маппинга /dev/mapper/cryptlvm.
#------------------------------------------------------------------------------
# 🧹 Очистка экрана
clear
echo " "
...
#------------------------------------------------------------------------------
# ШАГ 3: НАСТРОЙКА LVM (МЕНЕДЖЕР ЛОГИЧЕСКИХ ТОМОВ)
#------------------------------------------------------------------------------
# LVM позволяет гибко управлять разделами внутри зашифрованного контейнера.
# pvcreate — создание физического тома (Physical Volume).
# vgcreate — создание группы томов (Volume Group) с именем vg_main.
# lvcreate — создание логических томов (Logical Volumes).
#------------------------------------------------------------------------------
...
# 📦 Создание логического тома для swap (размер 4ГБ)
# ⚠️ Для гибернации размер swap должен быть >= объему RAM!
# Если у вас 8GB+ RAM, замените 4G на 8G или больше.
...
#------------------------------------------------------------------------------
# ШАГ 4: ФОРМАТИРОВАНИЕ BTRFS И СОЗДАНИЕ ПОДТОМОВ
#------------------------------------------------------------------------------
# Btrfs — современная ФС с поддержкой снапшотов и сжатия.
# Подтомы (@, @home и др.) позволяют создавать независимые снапшоты
# для разных частей системы и упрощают восстановление.
#------------------------------------------------------------------------------
...
# 📦 ✅ ИСПРАВЛЕНО: Создание подтомов BTRFS
# ОШИБКА В СТАРОЙ ВЕРСИИ: Команда 'btrfs su cr' была некорректной.
# ИСПРАВЛЕНИЕ: Используется полная команда 'btrfs subvolume create'.
# @ — корневая система (снапшоты системы без /home)
btrfs subvolume create /mnt/@
# @home — домашние каталоги пользователей
btrfs subvolume create /mnt/@home
# @log — системные логи (исключаются из снапшотов корня)
btrfs subvolume create /mnt/@log
# @pkg — кэш пакетов pacman (исключается для экономии места)
btrfs subvolume create /mnt/@pkg
# Отмонтирование для правильного монтирования подтомов
umount /mnt
#------------------------------------------------------------------------------
# ШАГ 5: МОНТИРОВАНИЕ РАЗДЕЛОВ И ПОДТОМОВ
#------------------------------------------------------------------------------
# Монтируем подтомы с опциями по умолчанию.
# Позже в fstab можно добавить сжатие и оптимизации.
#------------------------------------------------------------------------------
# 📦 Монтирование подтома @ как корень системы
mount -o defaults,subvol=@ /dev/vg_main/lv_root /mnt
# 📦 Создание необходимых директорий для точек монтирования
...
clear
echo " "
# 🔍 Проверка структуры разделов и шифрования
...
# plymouth-quit — корректное завершение экрана загрузки
systemctl enable NetworkManager.service grub-btrfsd.service sshd.service plymouth-quit-wait.service
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
# mkinitcpio создает initramfs — временную ФС, загружаемую до корневой.
# Для LUKS+LVM критически важен порядок хуков и наличие модулей.
#------------------------------------------------------------------------------
# 📋 Настройка HOOKS mkinitcpio — ПОРЯДОК КРИТИЧЕН!
# keyboard/keymap — ввод пароля LUKS с клавиатуры
# encrypt — расшифровка LUKS контейнера
# lvm2 — активация логических томов LVM
# resume — восстановление из гибернации (swap)
# filesystems — монтирование корневой ФС
# ✅ ИСПРАВЛЕНО: Используем \(.*\) для корректного захвата строки
...
# btrfs — поддержка файловой системы
# dm_mod/dm_crypt — Device Mapper для LVM и LUKS
...
#------------------------------------------------------------------------------
# Ядро должно получить параметры для расшифровки и монтирования корня.
# cryptdevice — указывает LUKS контейнер и имя маппинга.
# root — указывает корневой логический том LVM.
# rootflags — монтирует подтом @ BTRFS.
# resume — указывает swap для гибернации.
#------------------------------------------------------------------------------
# 📋 Получение UUID разделов для параметров ядра
# CRYPT_UUID — UUID зашифрованного раздела (sda3)
# ROOT_UUID — UUID логического тома root (lv_root)
# SWAP_UUID — UUID логического тома swap (lv_swap)
...
# 📋 ✅ ИСПРАВЛЕНО: Настройка GRUB_CMDLINE_LINUX
# ОШИБКИ В СТАРОЙ ВЕРСИИ:
# 1. Использовалась точка (.) вместо (.*) — заменялся только 1 символ.
# 2. Кавычки внутри строки не были экранированы — команда ломалась.
# ИСПРАВЛЕНИЯ:
# 1. Используется .* для замены всей строки.
# 2. Внутренние кавычки экранированы как \".
# 3. Гарантирует корректную передачу всех параметров ядру.
...
# Для BIOS с отдельным /boot это не требуется, так как GRUB читает
# ядро и initramfs из открытого раздела /boot. Пароль запрашивает initramfs.
...
# Создает файл /boot/grub/grub.cfg на основе настроек
grub-mkconfig -o /boot/grub/grub.cfg
# 📋 Пересборка initramfs
# Применяет изменения в mkinitcpio.conf и создает образы initramfs
mkinitcpio -P
# 🧹 Очистка экрана терминала
clear
echo " "
